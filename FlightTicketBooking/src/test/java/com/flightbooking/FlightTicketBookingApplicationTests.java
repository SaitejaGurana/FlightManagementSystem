...
class FlightTicketBookingApplicationTests {
	...
	{
		when(flightRepository.findAll()).thenReturn(Stream.of(new Flight(2,"abc",123,444.00),new Flight(2,"abc",234,555.00)).collect(Collectors.toList()));
	...
	public void addAirportTest() {
	  ...
	  @Test
	    public void getAirportsTest() {
	        when(airportRepository.findAll()).thenReturn(Stream.of(
	                new Airport(1, "abc", "ijk", "pqr"),
	                new Airport(2, "hhh", "ijk", "pqr"))
	...
	public void findByFlightIdTest() {
		Flight flight = new Flight(2, "pqw", 123, 444.00);
		when(flightRepository.findById(3)).thenReturn(Optional.of(flight));
		FlightDTO flightDTO = flightServiceImpl.viewByFlightId(2);
